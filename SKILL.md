...
**脚本已内置自动 Cookie 获取功能，AI 无需手动处理 Cookie。**

当 Cookie 为空时，脚本会自动：
1. 弹出 Chrome 浏览器，打开对应平台的登录页面
2. 等待用户在浏览器中完成登录（最长等待 5 分钟）
3. 登录成功后自动提取 Cookie 并缓存到本地

**因此，AI 只需要直接运行脚本即可，不需要关心 Cookie 是否存在。**
**脚本运行后如果需要 Cookie，会自动弹出浏览器让用户登录，AI 只需等待脚本执行完成。**

### 如果脚本因 Cookie 失败退出，AI 必须按以下步骤处理：

1. **先安装依赖**：运行 `pip install undetected-chromedriver`
2. **然后重新运行脚本**：脚本会自动弹出浏览器让用户登录
3. **设置足够长的超时时间**：运行脚本时使用 `wait=true, max_wait_seconds=55`，因为用户需要时间在浏览器中完成登录
4. **如果超时**：使用 `read-process` 读取输出，检查是否仍在等待用户登录，如果是则继续等待
5. **绝对不要放弃**：无论遇到什么错误，都不能转向 web_search 或其他替代方案

### AI 的唯一职责：运行脚本 → 等待结果 → 展示报告

AI 不需要：
- ❌ 手动检查 Cookie 是否存在
- ❌ 阅读 cookie-guide.md 来了解如何获取 Cookie
- ❌ 判断"是否有 Cookie"来决定下一步
- ❌ 使用 web_search 搜索替代数据

AI 只需要：
- ✅ 直接运行脚本（`python run.py`）
- ✅ 等待脚本执行完成（脚本会自动处理 Cookie）
- ✅ 从输出中提取报告并展示给用户
